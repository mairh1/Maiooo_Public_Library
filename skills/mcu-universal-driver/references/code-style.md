...
`mcu-universal-driver` skill 的参考文档：架构约束（C1–C10）见 [SKILL.md](../SKILL.md)，本文给出代码规范（S1–S5）的完整规则与 ❌/✅ 对照示例。示例取材自仓库项目真实代码，规则本身适用于任何 MCU 项目。本文为深度参考，与正文冲突时以正文为准。
...
### S1.6 驱动目录命名：`<芯片型号>_Driver`（仅芯片驱动适用）

规则只约束一种场景：**一颗外挂芯片对应一个驱动目录**，目录命名为 `<芯片型号>_Driver`，型号大小写与厂商数据手册 / 丝印一致。

✅：

```
driver/SGM41513_Driver/     # SGM41513 充电管理芯片
driver/BQ25622e_Driver/     # BQ25622E 充电芯片，'e' 小写按厂商丝印
```

❌ 型号未按丝印大小写、后缀拼错或使用复数：

```
driver/bq25622e_driver/     # 型号未按丝印大写
driver/AW32257_Drivers/     # 复数后缀，规范为单数 _Driver
```

> 适用边界：本条**仅适用于芯片驱动**。协议栈类组件（如按协议命名的 `usb_pd_driver/`）不是"一颗芯片一个驱动"，middleware、skills 等其它目录同理——均不适用 `<型号>_Driver` 规则，可用全小写 snake_case 或「协议名 + `_Driver` 后缀」。目录内的文件名也不受影响，仍按 S1 表「文件名」行全小写 + 下划线。
...
规则正文见 [SKILL.md](../SKILL.md) 的 S5。检查时通读新增或修改的代码，确认控制流程与关键计算可直接理解，命名与注释足以说明意图；禁止为压缩输出而生成晦涩代码，禁止无需求依据的测试脚手架与自证校验。
...
并告知用户：`.clang-format` 只能覆盖 S2 格式类规则；S1 命名、S3 注释、S4 ISR 与共享数据、S5 可读性是任何格式化工具都做不了的——这些正是需要 AI 按 skill 执行的部分。
