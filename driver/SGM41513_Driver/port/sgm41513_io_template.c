/**
 * @file    sgm41513_io_template.c
 * @brief   SGM41513 驱动的移植模板
 * @details 移植步骤（3 步）：
 *          1. 把本文件复制进你的工程（文件名随意，如 sgm41513_io.c）。
 *          2. 用你平台的 I2C API 填写下面的 io 契约函数（必选 4 个；
 *             SGM41513_THREAD_SAFE=1 时追加 lock/unlock 2 个可选）。
 *          3. 需要时调整 sgm41513_conf.h（变体、功能开关等）。
 *
 *          驱动只做单字节寄存器访问，无需支持块传输。dev_addr 是 7 位
 *          地址（0x1A）。
 *
 *          每个函数底部以注释形式给出两份已填好的示例（STM32 HAL 与
 *          ESP-IDF）；用不到的删掉即可。
 * @note    全部 io 契约函数均须在线程上下文调用，禁止在 ISR 中调用。
 * @author  Maiooo
 * @version 1.0.0
 * @date    2026-09-10
 */
