...
| 移植 | 只需实现 `sgm41513_io.h` 中的 4 个必选函数 + 2 个可选函数（`SGM41513_THREAD_SAFE=1` 时，见下文） |
...
|  寄存器   sgm41513_regs.h                   |
+--------------------------------------------+
        |  sgm41513_io.h      （移植接口，4 必选 + 2 可选函数）
+--------------------------------------------+
...
| `SGM41513_USE_JEITA` | 1 | JEITA 温度窗口配置；置 0 后相关 API 随 `#if` 从编译中移除（编译期报错） |
...
`ERR_NOT_SUPPORTED` 预留——当前功能裁剪走编译期移除（置 0 后对应 API 声明与
实现随 `#if` 一同从编译中移除），此结果码暂无产生路径；`ERR_VERIFY` 写入校验失败。
...
- **默认（`SGM41513_THREAD_SAFE = 0`）**：不同实例（不同 `sgm41513_dev_t`）之间
  并发调用安全；**同一实例**的并发调用需要外部保护（互斥锁或任务隔离）。
- **`SGM41513_THREAD_SAFE = 1`**：移植层必须实现 `sgm41513_io_lock()` /
  `sgm41513_io_unlock()`（典型实现为 RTOS 互斥锁 take/give），驱动核心在每个
  多笔寄存器访问的临界区前后调用它们。锁覆盖同一实例内的读-改-写序列；
  不同实例之间仍互不干扰。
- `sgm41513_init()` / `sgm41513_reset()` / `sgm41513_enter_ship_mode()` 等
  初始化与复位类调用与运行期调用的**并发不受支持**——由调用者保证时序
  （init 完成后再开始运行期调用；复位/进入船运模式期间不得并发调用其他 API）。

### 7.2 ISR 约束

- nINT 中断服务程序只置标志（`volatile`），不做数据处理；
- 状态读取与故障清除（`sgm41513_get_status()` / `sgm41513_get_faults()` /
  `sgm41513_feed_watchdog()` 等）必须在线程上下文调用；
- 驱动的所有 io 契约函数（I2C 读写、延时、lock/unlock）均**非 ISR 安全**，
  因此全部公共 API 都禁止在 ISR 中调用。
...
| `sgm41513_io.h` | 移植接口声明（4 必选 + 2 可选函数） |
