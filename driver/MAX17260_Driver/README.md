...
**线程安全（默认）**：不同 `max17260_dev_t` 实例之间并发安全；同一实例的并发访问需外部保护。置
`MAX17260_THREAD_SAFE=1` 后，驱动在多笔总线访问的 API 前后调用 `max17260_io_lock/unlock()`
（由移植层实现，如 RTOS 互斥量）。`init`/`deinit` 与运行期调用的并发不支持，由调用者保证时序。
...
### 2.1 配置宏（max17260_conf.h）

全部宏集中在 `max17260_conf.h`，均带 `#ifndef` 默认值，可被编译器 `-D` 覆盖；置 0 关闭功能开关后对应 API 与 io 契约函数一起被 `#if` 裁剪。

| 宏 | 默认值 | 说明 |
| --- | --- | --- |
| `MAX17260_VARIANT` | `MAX17260_VARIANT_DEFAULT` | 器件变体，决定 I2C 7 位地址；取值须为下表两个常量之一，非法值编译报错 |
| `MAX17260_VARIANT_DEFAULT` | 0 | SEWL+ / SETD+ 系列，I2C 地址 0x36 |
| `MAX17260_VARIANT_B_SERIES` | 1 | BEWL+ 系列，I2C 地址 0x0D |
| `MAX17260_RSENSE_MOHM` | 10 | 检流电阻毫欧值（典型 10，范围 1~1000）；填 0 时相关 API 返回原始 µVh/µV，见已知坑第 5/6 条 |
| `MAX17260_USE_SERIAL_NUMBER` | 1 | 置 1 提供 `read_serial()`；置 0 连同 AtRateEn/DPEn 切换逻辑一起裁剪 |
| `MAX17260_VERIFY_WRITES` | 0 | 置 1 写后回读校验（Status/序列号/只读寄存器自动排除），不一致返回 `ERR_VERIFY`，量产默认关闭 |
| `MAX17260_THREAD_SAFE` | 0 | 置 1 在多笔总线访问 API 前后调用 `max17260_io_lock/unlock()`（移植层实现）；单线程保持 0 零开销 |
