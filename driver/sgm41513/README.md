...
|  应用层                                     |  你的代码
+--------------------------------------------+
        |  sgm41513.h        （公共 API，≈ ff.h）
+--------------------------------------------+
|  驱动核心 sgm41513.c                        |  可移植，不含任何
|  配置     sgm41513_conf.h  （≈ ffconf.h）    |  平台代码
|  寄存器   sgm41513_regs.h                   |
+--------------------------------------------+
        |  sgm41513_io.h      （移植接口，≈ diskio.h，仅 4 个函数）
+--------------------------------------------+
...
| FatFS | 本驱动 |
|---|---|
| `ff.h` / `ff.c` | `sgm41513.h` / `sgm41513.c` |
| `ffconf.h` 功能裁剪 | `sgm41513_conf.h` |
| `diskio.h` / `diskio.c` | `sgm41513_io.h` / `port/sgm41513_io_template.c` |
| `FRESULT` 错误码 | `sgm41513_result_t` |
| `FF_FS_REENTRANT` 互斥钩子 | `SGM41513_THREAD_SAFE` + `sgm41513_io_lock/unlock` |
