/*
 * @file    max17048_io_template.c
 * @brief   MAX17048/MAX17049 驱动移植模板
 * @details 移植三步走：
 *          1. 把本文件复制进工程（可改名 max17048_io.c）；
 *          2. 用目标平台 I2C API 填空下方函数（默认配置 4 个，其中毫秒
 *             延时仅 MAX17048_USE_MODEL_TABLE=1 时编译；含两个平台的
 *             参考实现注释，用不到的删掉）；
 *          3. 按需调整 max17048_conf.h（变体、功能开关等）。
 * @note    器件全部寄存器为 16 位，两字节必须在同一 I2C 事务内传输，
 *          高字节在前（CH32/STM32 HAL 的 Mem_Read/Mem_Write 天然满足）；
 *          dev_addr 为 7 位地址（0x36），多数 HAL 需左移一位后传入。
 * @author  Maiooo
 * @version 1.0.0
 * @date    2026-08-25
 */
 ...
 * 1. 总线初始化（可选）
 * ════════════════════════════════════════════════════════════════════════ */
/* max17048_init() 会调用一次；I2C 在别处初始化时直接返回 OK 即可。 */
                           ...
                           uint16_t *val)
{
    ...
    *val = 0u;
    return MAX17048_IO_ERROR;   /* 桩：尚未移植 */
}
...
/* ══════════════════════════════════════════════════════════════════════════
 * 4. 毫秒延时（仅 MAX17048_USE_MODEL_TABLE = 1 时编译）
 * ════════════════════════════════════════════════════════════════════════ */
/* 仅模型表加载序列使用（MAX17048_USE_MODEL_TABLE=1），其余配置可放空。
 * 实际延时不得小于请求值。 */
...
/* ══════════════════════════════════════════════════════════════════════════
 * 5. 并发保护钩子（仅 MAX17048_THREAD_SAFE = 1 时编译）
 * ════════════════════════════════════════════════════════════════════════ */
