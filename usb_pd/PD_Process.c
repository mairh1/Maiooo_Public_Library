...
{
    if(USBPD_GetStatusFlag(IF_RX_ACT))
    {
        USBPD_ClearStatusFlag(IF_RX_ACT);
        if( USBPD_GetReceiveStatus() == PD_RX_SOP0 )
        {
            if( USBPD->BMC_BYTE_CNT >= 6 )
            {
                ...
                if( ( USBPD->BMC_BYTE_CNT != 6 ) || ( ( PD_Rx_Buf[ 0 ] & 0x1F ) != DEF_TYPE_GOODCRC ) )
                {
                    ...
                    USBPD_ITConfig(IE_TX_END, ENABLE);
                    USBPD_PHY_SendPack( 0, PD_Ack_Buf, 2, UPD_SOP0 );
                }
            }
        }
    }
    if(USBPD_GetStatusFlag(IF_TX_END))
    {
        /* Packet send completion interrupt (GoodCRC send completion interrupt only) */
        USBPD_CCCmdConfig(USBPD_PORT_CC1, DISABLE, CC_LVE);
        USBPD_CCCmdConfig(USBPD_PORT_CC2, DISABLE, CC_LVE);
        ...
        USBPD_ClearStatusFlag(IF_TX_END);
    }
    if(USBPD_GetStatusFlag(IF_RX_RESET))
    {
        USBPD_ClearStatusFlag(IF_RX_RESET);
        PD_SINK_Init( );
        printf("IF_RX_RESET\r\n");
    }
    if(USBPD_GetStatusFlag(BUF_ERR))
    {
        printf("BUFERR\r\n");
    }
}
...
void PD_Init( void )
{
    ...
    USBPD_InitTypeDef USBPD_InitStruct;

    /* GPIO Configuration: PB6/CC1, PB7/CC2 */
    ...
    /* Enable high threshold input mode for PD pins */
    AFIO->CR |= USBPD_IN_HVT ;

    /* USBPD peripheral initialization */
    USBPD_StructInit(&USBPD_InitStruct);
    USBPD_InitStruct.USBPD_FiltEn = 1;                                         /* Enable input filter */
    USBPD_InitStruct.USBPD_DMAEn = 1;                                          /* Enable DMA */
    USBPD_Init(&USBPD_InitStruct);

    /* Clear all pending interrupt flags */
    USBPD_ClearStatusFlag(BUF_ERR | IF_RX_BIT | IF_RX_BYTE | IF_RX_ACT | IF_RX_RESET | IF_TX_END);
...
UINT8 PD_Detect( void )
{
    ...
    {
        /* Configure CC1 comparator to 0.22V and read analog input */
        USBPD_SetCCComparator(USBPD_PORT_CC1, CC_CMP_22);
        Delay_Us(2);
        if( USBPD_GetCCAnalogInput(USBPD_PORT_CC1) )
        {
            cmp_cc1 |= bCC_CMP_22;
        }

        /* Configure CC2 comparator to 0.22V and read analog input */
        USBPD_SetCCComparator(USBPD_PORT_CC2, CC_CMP_22);
        Delay_Us(2);
        if( USBPD_GetCCAnalogInput(USBPD_PORT_CC2) )
        {
            cmp_cc2 |= bCC_CMP_22;
        }

        /* Check if CC pull-down is enabled (SNK mode) */
...
void PD_Det_Proc( void )
{
    ...
    else
    {
        ...
        if( PD_Ctl.Det_Cnt >= 5 )
        {
            ...
            {
                /* Check if either CC port has pull-down enabled */
...
{
    USBPD_PHY_SendPack( mode, pbuf, len, sop );
}
...
UINT8 PD_Send_Handle( UINT8 *pbuf, UINT8 len )
{
    ...
    {
        NVIC_DisableIRQ( USBPD_IRQn );
        USBPD_PHY_SendPack( 0x01, PD_Tx_Buf, ( len + 2 ), UPD_SOP0 );
